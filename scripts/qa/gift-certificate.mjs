...
const out = 'docs/qa/gift-certificate';
...
try {
  for (const width of [375, 800, 1440]) {
    ...
    const requests = [];
    page.on('request', r => { if (r.url().includes('/certificate-mobile-20260922/')) requests.push(r.url()); });
    ...
    if (width <= 800) await section.screenshot({ path: `${out}/${width}.png`, style: 'header.header, .mobile-cta-bar, .cookie-notice { visibility: hidden !important; }' });
