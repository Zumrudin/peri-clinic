...
**Прод и стенд с 2026-09-17 (фаза 8):** продакшен на `217.114.0.254` ходит в базу
`peri_clinic_site` напрямую (без туннеля, TLS с закреплённым корневым CA Beget — см.
`deploy/directus.env.example`). Стенд переведён в **отдельную базу `peri_clinic_site_dev`**
(та же роль, тот же инстанс; базу создал заказчик в панели Beget, потому что у роли нет
`CREATEDB`). Содержимое — копия прода на 2026-09-17: промежуточно стенд сутки жил в схеме `dev`
той же базы (`DB_SEARCH_PATH=dev`), затем эта схема была перелита в новую базу
(`pg_dump --schema=dev` → префикс схемы переписан на `public` вне COPY-блоков →
`psql --single-transaction`; таблицы, строки и sequence сверены 1:1) и удалена из прода.
Дампы обеих операций — `/srv/peri/backups/dev-schema-2026-09-16T17-02-32/` и
`/srv/peri/backups/dev-db-2026-09-17T06-26-32/`. Правки редакторов на стенде **не попадают**
на прод и наоборот. Роль общая, поэтому пароль один на обе базы.

**Ловушка pm2:** не запускать `pm2 restart --update-env` из шелла, где до этого сделан
`set -a; . .env` — pm2 сохраняет переменные шелла в окружение процесса, а `.env`-файл их
потом не перекрывает (так стенд один раз остался на `DB_SEARCH_PATH=dev` после удаления строки из
файла). После правки `.env` — `pm2 restart peri-directus` из чистого шелла, а если утечка уже
случилась — `pm2 delete` + `pm2 start ecosystem.config.cjs --only …` + `pm2 save`.
