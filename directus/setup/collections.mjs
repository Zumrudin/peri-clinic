...
export const collections = [
  {
    ...
    fields: [
      ...
      f.str('nearest_metro', 'Ближайшее метро', { width: 'half' }),
      f.str('metro_walk_time', 'Время пешком от метро', { width: 'half', note: 'Например: 7 минут пешком' }),
      f.text('metro_directions', 'Как пройти от метро', { note: 'Короткий понятный маршрут для страницы контактов и поисковиков' }),
...

  {
    ...
    fields: [
      ...
      f.repeater('reviews_platforms', 'Кнопки площадок с отзывами', [
        { field: 'label', type: 'string', name: 'Подпись', meta: { interface: 'input' } },
        { field: 'url', type: 'string', name: 'HTTPS-ссылка на отзывы', meta: { interface: 'input' } },
      ], { template: '{{label}}', note: 'Яндекс Карты, Google Maps, 2ГИС. Указывайте карточку именно нашей клиники.' }),
      f.str('reviews_rating_label', 'Подпись рейтинга', { width: 'half', default: 'рейтинг клиники' }),
      f.divider('d_gift', 'Подарочный сертификат · мобильная версия'),
      f.text('gift_eyebrow', 'Надзаголовок'),
      f.text('gift_title', 'Заголовок'),
      f.text('gift_lead', 'Описание'),
      f.text('gift_button_label', 'Текст кнопки'),
      f.text('gift_context', 'Тема обращения'),
      f.text('gift_video_url', 'Ссылка на видео MP4 (HTTPS)'),
      f.text('gift_poster_url', 'Ссылка на постер (HTTPS)'),
      f.text('gift_video_label', 'Описание видео'),
      f.text('gift_play_label', 'Кнопка воспроизведения'),
      f.text('gift_pause_label', 'Кнопка паузы'),
...

  {
    ...
    fields: [
      ...
      f.str('source_url', 'Ссылка на оригинал отзыва', { width: 'half', note: 'Скопируйте ссылку через «Поделиться» у отзыва.' }),
      { ...f.str('external_id', 'ID отзыва во внешней системе'), schema: { is_unique: true, is_nullable: true } },
      f.bool('show_on_home', 'Показывать на главной', false),
      ...timestamps(),
    ],
  },
